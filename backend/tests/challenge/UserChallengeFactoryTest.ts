...
describe("GoalInstanceFactory test", () => {
    ...
    describe('Constructor', () => {
        it("should have proper name when built", () => {
            chai.expect(userChallenge.getName()).to.be.equal(aGoalName);
        });

        it("should have proper user when built", () => {
            chai.expect(userChallenge.getUser().getName()).to.be.equal(aUser.getName());
        });


        it('should have proper startDate when built', () => {
            var expectedStartDate =  moment('2015-08-24T00:00:00');
            //  Because challenge taken during a session are "realigned" with this session

            chai.expect(userChallenge.getStartDate().toISOString()).to.be.eql(expectedStartDate.toISOString());

        });

        it('should have proper endDate when built', () => {
            var expectedEndDate =  moment('2015-08-28T23:59:59.999');
            //  Because challenge taken during a session are "realigned" with this session

            chai.expect(userChallenge.getEndDate().toISOString()).to.be.eql(expectedEndDate.toISOString());
        });
    });
